...
public class AuthService {
    ...
    public AuthResponse register(@Valid User request) {
        ...
        if (existingUserByEmail.isPresent()) {
            throw new AlreadyExistsException(Constans.USER_EMAIL_EXISTS.getMessage());
        }
    ...
    public AuthResponse login(LoginRequest request) {
        try {
            ...
            throw new AuthenticationFailedException(Constans.CREDENTIAL_INVALID.getMessage());
        }
        UserDetails user = userRepository.findByEmail(request.getEmail()).
                orElseThrow(() -> new NotFoundException(Constans.CREDENTIAL_INVALID.getMessage()));
